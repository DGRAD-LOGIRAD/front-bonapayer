...
export default function UserModal({ visible, onClose }: UserModalProps) {
    ...
    const [errors, setErrors] = useState<{ login?: string; mail?: string; fkAgent?: string; backend?: string }>({});
    ...
    const handleSave = () => {
      ...
      //  const newErrors: any = {};
        ...
        axios.post(`${getBaseUrl()}/api-utilisateur/v1/saveCompte`, data, {
            headers: { Authorization: "Bearer 123" },
        })
        .then(() => {
                        ...
                        <div className="mb-4">
                            ...
                            {agentInfo ? (
                                <div className="border rounded-md p-3 bg-gray-50">
                                    <p><strong>Nom complet :</strong> {agentInfo.nomComplet}</p>
                                    <p><strong>Site :</strong> {agentInfo.fk_site_intitule}</p>
                                    <p><strong>Fonction :</strong> {agentInfo.fk_fonction_intitule}</p>
                                    <p><strong>Téléphone :</strong> {agentInfo.telephone}</p>
                                </div>
                            ) : (
                                <p className={`italic ${errors.fkAgent ? "text-red-500" : "text-gray-400"}`}>
                                    Aucun agent sélectionné
                                </p>
                            )}
                            {errors.fkAgent && <p className="text-red-500 text-sm mt-1">{errors.fkAgent}</p>}
                            <Button
                                    ...
                                    <tbody>
                                        {paginatedAgents.map((agent, idx) => (
                                            <tr key={agent.id} className="hover:bg-gray-100">
                                                ...
                                                <td className="p-2 border">{agent.fk_fonction_intitule}</td>
